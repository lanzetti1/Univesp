...
exports.default = {
     ...
     */
    async sendRawTransaction(address, { data, value }, txParams = {}, retries = RETRY_COUNT) {
        await this._fixGasPrice(txParams);
        try {
            const from = await ZWeb3_1.default.defaultAccount();
     ...
     * @param retries number of transaction retries
     */
    async sendTransaction(contractFn, args = [], txParams = {}, retries = RETRY_COUNT) {
        await this._fixGasPrice(txParams);
        try {
            const gas = txParams.gas ||
                Contracts_1.default.getArtifactsDefaults().gas ||
     ...
     */
    async deployContract(contract, args = [], txParams = {}, retries = RETRY_COUNT) {
        await this._fixGasPrice(txParams);
        try {
            const gas = txParams.gas ||
                Contracts_1.default.getArtifactsDefaults().gas ||
                (await this.estimateActualGas(Object.assign({ data: ABIs_1.buildDeploymentCallData(contract, args) }, txParams)));
    ...
    async estimateGas(txParams, retries = RETRY_COUNT) {
        ...
        try {
            ...
            return await ZWeb3_1.default.eth.estimateGas(txParamsWithoutGas);
        }
        catch (error) {
            if (retries <= 0)
                throw Error(error);
            await sleep_1.default(RETRY_SLEEP_TIME);
            return await this.estimateGas(txParams, retries - 1);
        }
    },
    async estimateActualGasFnCall(contractFn, args, txParams, retries = RETRY_COUNT) {
        // Retry if estimate fails. This could happen because we are depending
        // on a previous transaction being mined that still hasn't reach the node
        // we are working with, if the txs are routed to different nodes.
        // See https://github.com/zeppelinos/zos/issues/192 for more info.
        try {
            return await this._calculateActualGas(await contractFn(...args).estimateGas(Object.assign({}, txParams)));
        }
        catch (error) {
            if (retries <= 0)
                throw Error(error);
    ...
    async estimateActualGas(txParams) {
        const estimatedGas = await this.estimateGas(txParams);
        return this._calculateActualGas(estimatedGas);
    },
    async awaitConfirmations(transactionHash, confirmations = 12, interval = 1000, timeout = 10 * 60 * 1000) {
        if (await ZWeb3_1.default.isGanacheNode())
            return;
        ...
        while (true) {
            ...
            const currentBlock = await ZWeb3_1.default.getLatestBlockNumber();
            const txBlock = await getTxBlock();
            if (currentBlock - txBlock >= confirmations)
                return true;
            await sleep_1.default(interval);
        }
    },
    async _getETHGasStationPrice() {
        if (exports.state.gasPrice)
            return exports.state.gasPrice;
        try {
            const { data: responseData } = await axios_1.default.get(GAS_API_URL);
    ...
    async _fixGasPrice(txParams) {
        const gasPrice = txParams.gasPrice || Contracts_1.default.getArtifactsDefaults().gasPrice;
        if ((TRUFFLE_DEFAULT_GAS_PRICE.eq(gasPrice) || !gasPrice) && (await ZWeb3_1.default.isMainnet())) {
    ...
    async _getBlockGasLimit() {
        ...
        exports.state.block = await ZWeb3_1.default.getLatestBlock();
        return exports.state.block.gasLimit;
    },
    async _calculateActualGas(estimatedGas) {
        const blockLimit = await this._getBlockGasLimit();
