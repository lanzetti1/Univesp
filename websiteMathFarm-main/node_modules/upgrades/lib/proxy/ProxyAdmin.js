...
class ProxyAdmin {
    ...
    static async deploy(txParams = {}) {
        ...
        const contract = await Transactions_1.default.deployContract(Contracts_1.default.getFromLib('ProxyAdmin'), [], txParams);
    ...
    async changeProxyAdmin(proxyAddress, newAdmin) {
        ...
        await Transactions_1.default.sendTransaction(this.contract.methods.changeProxyAdmin, [proxyAddress, newAdmin], Object.assign({}, this.txParams));
        Logger_1.Loggy.succeed('change-proxy-admin', `Admin for proxy ${proxyAddress} set to ${newAdmin}`);
    }
    async upgradeProxy(proxyAddress, implementationAddress, contract, initMethodName, initArgs) {
    ...
    async transferOwnership(newAdminOwner) {
        ...
        await Transactions_1.default.sendTransaction(this.contract.methods.transferOwnership, [newAdminOwner], Object.assign({}, this.txParams));
        Logger_1.Loggy.succeed('transfer-ownership', `Proxy admin owner set to ${newAdminOwner}`);
    }
    async getOwner() {
        return await this.contract.methods.owner().call(Object.assign({}, this.txParams));
    }
    async checkOwner() {
    ...
    async _upgradeProxy(proxyAddress, implementation) {
    ...
    async _upgradeProxyAndCall(proxyAddress, implementationAddress, contract, initMethodName, initArgs) {
